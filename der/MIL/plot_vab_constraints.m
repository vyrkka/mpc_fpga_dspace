function [f, g] = plot_vab_constraints(R, L, show, s1, s2)
    ...
    if show
        ...
        title('Output Voltage Hard Constraints')
        ...
        % Plot signal
        plot(s1(1,:), s1(2,:), 'Color',[0.0 0.45 0.74], 'LineWidth', 1, 'DisplayName','MPC');
        %plot(s1(1,1), s1(2,1), 'ko', 'MarkerFaceColor','k', 'HandleVisibility','off');

        % Plot signal
        plot(s2(1,:), s2(2,:), 'Color',[0.85 0.33 0.10], 'LineWidth', 1, 'DisplayName','DNN');
        %plot(s2(1,1), s2(2,1), 'ko', 'MarkerFaceColor','k', 'HandleVisibility','off');
        
        for j = 1:L
            Z = f(j,1) * X + f(j,2) * Y - g(j);
            contour(X, Y, Z, [0 0], 'k', 'LineWidth', 3.5, 'HandleVisibility','off');
        end
        ...
        xlim([-1.1*R 1.1*R]);
        ylim([-1.1*R 1.1*R]);
        xlabel('$v_{s\alpha}$ [V]','Interpreter','latex')
        ylabel('$v_{s\beta}$ [V]','Interpreter','latex')
