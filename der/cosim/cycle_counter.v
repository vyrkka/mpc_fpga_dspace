module cycle_counter (
	...
	reg ap_ready_prev = 'd0;
	always @(posedge clk_1) begin
		ap_ready_prev <= ap_ready;
	end
	wire ap_ready_valid = ap_ready_prev && ~ap_ready;
	
	reg ap_done_prev = 'd0;
	always @(posedge clk_1) begin
		ap_done_prev <= ap_done;
	end
	wire ap_done_valid = ap_done_prev && ~ap_done;
    ...
    always @* begin
        case (mode)
            ...
            2'b01: begin // Mode 2
                ...
                num_cycles = (ap_ready_valid) ? (cycle_count + 'd1) : 0;
            end
            2'b10: begin // Mode 3
                ap_start = (cycle_count == 'd0) ? 1 : 0;
                num_cycles = (ap_done_valid) ? (cycle_count + 'd1) : 0;
            end
    ...
    always @(posedge clk_1 or posedge reset) begin
        if (reset) begin
            ...
            if ((mode == 2'b01) && (~ap_ready_valid)) begin
                cycle_count <= cycle_count + 'd1;
            end else if ((mode == 2'b10) && (~ap_done_valid)) begin
                cycle_count <= cycle_count + 'd1;
            end else begin
				cycle_count <= 'd0;
			end
