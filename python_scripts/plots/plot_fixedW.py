import os
import re
import pandas as pd
import matplotlib.pyplot as plt

problem = 'der2'
L=4
M=30
Q_fixed = 4

if problem == 'der1':
    red = 'DNN-Z'
    labels_text = {'u0':'vsd', 'u1':'vsq', 'Average': 'average'}
elif problem == 'der2':
    red = 'DNN-P'
    labels_text = {'u0':'Vc', 'u1':'\u03c9c', 'Average': 'average'}
else:
    red = problem
    labels_text = {'u0':'u0', 'u1':'u1', 'Average': 'Average'}

baseline_filename = None

# Directory containing the CSV files (use '.' for current directory)
data_dir = f'{problem}/{problem}L{L}M{M}'
# -----------------------------------

pattern = re.compile(r'^statsL(?P<L>\d+)M(?P<M>\d+)W(?P<W>\d+)Q(?P<Q>\d+)\.csv$')

# Storage: dict of label -> list of (W, rmse)
values = {'u0': [], 'u1': [], 'Average': []}
found_L = None
found_M = None

for fname in os.listdir(data_dir):
    m = pattern.match(fname)
    if not m:
        continue
    Q = int(m.group('Q'))
    if Q != Q_fixed:
        continue  
    L = int(m.group('L')); M = int(m.group('M')); W = int(m.group('W'))
    # remember L and M for baseline detection
    if found_L is None:
        found_L, found_M = L, M

    # read CSV
    fullpath = os.path.join(data_dir, fname)
    try:
        df = pd.read_csv(fullpath, index_col=0)  # uses first column (u,r2,...) as index
    except Exception as e:
        print(f"Warning: could not read {fname}: {e}")
        continue

    # Expect index values 'u0','u1','Average' and column 'rmse'
    for label in ['u0', 'u1', 'Average']:
        if label in df.index and 'rmse' in df.columns:
            try:
                rmse_val = float(df.loc[label, 'rmse'])
                values[label].append((W, rmse_val))
            except Exception as e:
                print(f"Warning parsing rmse for {label} in {fname}: {e}")
        else:
            print(f"Warning: {label} or rmse column not found in {fname}")

# Convert to sorted lists by W
for label in values:
    values[label].sort(key=lambda x: x[0])  # sort by W
    # unzip into separate lists for plotting
    ws = [w for w, _ in values[label]]
    rmses = [r for _, r in values[label]]
    values[label] = (ws, rmses)

# Attempt to detect baseline file
baseline = {}
if baseline_filename is None and found_L is not None:
    candidate = f"statsL{found_L}M{found_M}.csv"
    if os.path.exists(os.path.join(data_dir, candidate)):
        baseline_filename = candidate

if baseline_filename is not None and os.path.exists(os.path.join(data_dir, baseline_filename)):
    try:
        df_base = pd.read_csv(os.path.join(data_dir, baseline_filename), index_col=0)
        for label in ['u0', 'u1', 'Average']:
            if label in df_base.index and 'rmse' in df_base.columns:
                baseline[label] = float(df_base.loc[label, 'rmse'])
            else:
                print(f"Baseline file missing {label} or rmse column: {baseline_filename}")
    except Exception as e:
        print(f"Could not read baseline file {baseline_filename}: {e}")
else:
    if baseline_filename is not None:
        print(f"Baseline file not found: {baseline_filename}")
    else:
        print("No baseline filename provided and none auto-detected. Skipping baseline.")

# Plotting
plt.figure(figsize=(9, 6))

styles = {
    'u0': {'marker': 'o', 'linestyle': '-', 'label': f"{labels_text['u0']} (fixed point Q={Q_fixed})"},
    'u1': {'marker': 's', 'linestyle': '-', 'label': f"{labels_text['u1']} (fixed point Q={Q_fixed})"},
    'Average': {'marker': '^', 'linestyle': '-', 'label': f"{labels_text['Average']} (fixed point Q={Q_fixed})"},
}

any_plotted = False
minW = None; maxW = None
for label in ['u0', 'u1', 'Average']:
    ws, rmses = values[label]
    if len(ws) == 0:
        print(f"No data found for {label} (Q={Q_fixed}).")
        continue
    plt.plot(ws, rmses, marker=styles[label]['marker'],
             linestyle=styles[label]['linestyle'], label=styles[label]['label'])
    any_plotted = True
    if minW is None:
        minW = min(ws); maxW = max(ws)
    else:
        minW = min(minW, min(ws)); maxW = max(maxW, max(ws))

if not any_plotted:
    raise SystemExit(f"No Q={Q_fixed} data found. Exiting.")

# Plot baseline horizontal lines (if available)
if baseline:
    xmin = minW
    xmax = maxW

    # Get the same colors used in the previous lines
    lines = plt.gca().get_lines()
    color_map = {
    'u0': lines[0].get_color(),
    'u1': lines[1].get_color(),
    'Average': lines[2].get_color()
    }
    for label, y in baseline.items():
        label_text = labels_text[label]
        if label in color_map:
            plt.hlines(
                y=y, xmin=xmin, xmax=xmax,
                colors=color_map[label],
                linestyles='--', linewidth=1.5,
                label=f'{label_text} (floating point)'
            )
        else:
            plt.hlines(
                y=y, xmin=xmin, xmax=xmax,
                linestyles='--', linewidth=1.5,
                label=f'{label_text} (floating point)'
            )

plt.xlabel('Total bit width W')
plt.ylabel('RMSE')
plt.title(f'RMSE vs W for {red} (fixing Q={Q_fixed})')
plt.grid(True, linestyle=':', alpha=0.7)
plt.legend()
plt.tight_layout()

outname = f'rmse_vs_W_Q{Q_fixed}_{problem}.png'
plt.savefig(outname, dpi=300)
print(f"Saved figure to {outname}")
plt.show()