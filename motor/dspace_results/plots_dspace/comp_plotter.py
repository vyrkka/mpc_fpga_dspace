...
plt.plot(t32[k_inf:k_sup], u_act32[k_inf+delta32:k_sup+delta32],
         linestyle='-', color='m', label='u (W=32)')
plt.plot(t16[k_inf:k_sup], u_act16[k_inf+delta16:k_sup+delta16],
         linestyle='-', color='b', label='u (W=16)')

# pos signals
plt.plot(t32[k_inf:k_sup], pos32[k_inf+delta32:k_sup+delta32],
         linestyle='-', color='r', label='pos (W=32)')
plt.plot(t16[k_inf:k_sup], pos16[k_inf+delta16:k_sup+delta16],
         linestyle='-', color='g', label='pos (W=16)')

# Add labels and title
plt.xlabel('Time [s]')
plt.ylabel('Data')
plt.title(f'Implicit MPC for servomotor (W=32 and W=16)')
plt.legend()

# Display the plot
plt.grid(True)
plt.show()
plt.savefig(f'plots/comp_fis_imp.png')